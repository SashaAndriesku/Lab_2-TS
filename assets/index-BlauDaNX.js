(function(){let e=document.createElement(`link`).relList;if(e&&e.supports&&e.supports(`modulepreload`))return;for(let e of document.querySelectorAll(`link[rel="modulepreload"]`))n(e);new MutationObserver(e=>{for(let t of e)if(t.type===`childList`)for(let e of t.addedNodes)e.tagName===`LINK`&&e.rel===`modulepreload`&&n(e)}).observe(document,{childList:!0,subtree:!0});function t(e){let t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),t.credentials=e.crossOrigin===`use-credentials`?`include`:e.crossOrigin===`anonymous`?`omit`:`same-origin`,t}function n(e){if(e.ep)return;e.ep=!0;let n=t(e);fetch(e.href,n)}})();var e=class{id;title;author;year;isBorrowed;borrowedBy;constructor(e,t,n,r,i=!1,a){this.id=e,this.title=t,this.author=n,this.year=r,this.isBorrowed=i,this.borrowedBy=a}borrow(e){this.isBorrowed=!0,this.borrowedBy=e}returnBook(){this.isBorrowed=!1,this.borrowedBy=void 0}},t=class{id;name;email;borrowedBookIds;constructor(e,t,n,r=[]){this.id=e,this.name=t,this.email=n,this.borrowedBookIds=r}canBorrow(){return this.borrowedBookIds.length<3}addBook(e){this.canBorrow()&&this.borrowedBookIds.push(e)}removeBook(e){this.borrowedBookIds=this.borrowedBookIds.filter(t=>t!==e)}},n=class{items=[];constructor(e=[]){this.items=e}add(e){this.items.push(e)}remove(e,t){let n=this.items.findIndex(t=>t.id===e);return n===-1||t&&!t(this.items[n])?!1:(this.items.splice(n,1),!0)}findById(e){return this.items.find(t=>t.id===e)}getAll(){return[...this.items]}clear(){this.items=[]}},r=class{static save(e,t){localStorage.setItem(e,JSON.stringify(t))}static load(e){let t=localStorage.getItem(e);if(!t)return null;try{return JSON.parse(t)}catch{return null}}static remove(e){localStorage.removeItem(e)}},i;(function(e){function t(e){return e.trim().length>0}e.isRequired=t;function n(e){return/^\d+$/.test(e.trim())}e.isDigitsOnly=n;function r(e){if(!n(e))return!1;let t=parseInt(e,10),r=new Date().getFullYear();return t>=1e3&&t<=r}e.isValidYear=r;function i(e){return/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(e.trim())}e.isValidEmail=i})(i||={});function a(){return Date.now().toString()+Math.floor(Math.random()*100).toString()}var o=class{static show(e,t,n=`Зрозуміло!`,r){let i=document.getElementById(`custom-modal`);i&&i.remove();let a=document.createElement(`div`);a.id=`custom-modal`,a.className=`modal fade show d-block`,a.style.backgroundColor=`rgba(0, 0, 0, 0.5)`,a.innerHTML=`
    ...
    `,document.body.appendChild(i);let a=()=>i.remove();document.getElementById(`modal-close-btn`)?.addEventListener(`click`,a),document.getElementById(`modal-cancel-btn`)?.addEventListener(`click`,a),document.getElementById(`modal-save-btn`)?.addEventListener(`click`,()=>{let e=document.getElementById(`modal-prompt-input`);if(e&&e.value.trim()){let t=e.value.trim();a(),n(t)}else{let e=document.getElementById(`modal-error`);e&&(e.textContent=`Це поле є обов’язковим`,e.classList.remove(`d-none`))}})}},s=class{bookLibrary;userLibrary;bookSearchQuery=``;currentBookPage=1;currentUsersPage=1;itemsPerPage=5;constructor(){let i=r.load(`books`)||[],a=r.load(`users`)||[];this.bookLibrary=new n(i.map(t=>new e(t.id,t.title,t.author,t.year,t.isBorrowed,t.borrowedBy))),this.userLibrary=new n(a.map(e=>new t(e.id,e.name,e.email,e.borrowedBookIds)))}saveData(){r.save(`books`,this.bookLibrary.getAll()),r.save(`users`,this.userLibrary.getAll())}init(e){this.renderLayout(e),this.attachEvents(),this.updateLists()}renderLayout(e){e.innerHTML=`
      ...
      </div>`}attachEvents(){document.getElementById(`book-form`)?.addEventListener(`submit`,e=>{e.preventDefault(),this.handleAddBook()}),document.getElementById(`user-form`)?.addEventListener(`submit`,e=>{e.preventDefault(),this.handleAddUser()}),document.getElementById(`book-search`)?.addEventListener(`input`,e=>{this.bookSearchQuery=e.target.value.toLowerCase(),this.currentBookPage=1,this.updateLists()}),document.getElementById(`book-list-container`)?.addEventListener(`click`,e=>{let t=e.target.closest(`button`);if(!t)return;let n=t.getAttribute(`data-id`)||``;t.classList.contains(`borrow-btn`)?this.handleBorrowBook(n):t.classList.contains(`return-btn`)?this.handleReturnBook(n):t.classList.contains(`delete-book-btn`)&&this.removeAndRefresh(this.bookLibrary,n,e=>(e.isBorrowed&&o.show(`Помилка`,`Неможливо видалити книгу, яка зараз видана користувачу!`),!e.isBorrowed))}),document.getElementById(`user-list-container`)?.addEventListener(`click`,e=>{let t=e.target.closest(`button`);if(t?.classList.contains(`delete-user-btn`)){let e=t.getAttribute(`data-id`)||``;this.removeAndRefresh(this.userLibrary,e,e=>{let t=e.borrowedBookIds&&e.borrowedBookIds.length>0;return t&&o.show(`Помилка`,`Неможливо видалити користувача, у якого є неповернуті книги!`),!t})}})}removeAndRefresh(e,t,n){e.remove(t,n)&&(this.saveData(),this.updateLists())}handleAddBook(){let t=document.getElementById(`book-title`),n=document.getElementById(`book-author`),r=document.getElementById(`book-year`),o=i.isRequired(t.value),s=i.isRequired(n.value),c=i.isValidYear(r.value);this.toggleError(`book-title-error`,!o),this.toggleError(`book-author-error`,!s),this.toggleError(`book-year-error`,!c),o&&s&&c&&(this.bookLibrary.add(new e(a(),t.value.trim(),n.value.trim(),parseInt(r.value.trim(),10))),t.value=n.value=r.value=``,this.saveData(),this.updateLists())}handleAddUser(){let e=document.getElementById(`user-name`),n=document.getElementById(`user-email`),r=i.isRequired(e.value),o=i.isValidEmail(n.value);this.toggleError(`user-name-error`,!r),this.toggleError(`user-email-error`,!o),r&&o&&(this.userLibrary.add(new t(a(),e.value.trim(),n.value.trim())),e.value=n.value=``,this.saveData(),this.updateLists())}toggleError(e,t){document.getElementById(e)?.classList.toggle(`d-none`,!t)}updateLists(){this.renderBooks(),this.renderUsers()}getPaginatedItems(e,t){let n=Math.ceil(e.length/this.itemsPerPage)||1,r=Math.min(t,n),i=(r-1)*this.itemsPerPage;return{items:e.slice(i,i+this.itemsPerPage),totalPages:n,page:r}}renderBooks(){let e=document.getElementById(`book-list-container`);if(!e)return;let t=this.bookLibrary.getAll();this.bookSearchQuery&&(t=t.filter(e=>e.title.toLowerCase().includes(this.bookSearchQuery)||e.author.toLowerCase().includes(this.bookSearchQuery)));let{items:n,totalPages:r,page:i}=this.getPaginatedItems(t,this.currentBookPage);this.currentBookPage=i,e.innerHTML=n.length===0?`<p class="text-muted">Книг не знайдено.</p>`:n.map(e=>`
      <div class="d-flex justify-content-between align-items-center border-bottom py-2">
        <div><strong>${e.title}</strong> by ${e.author} (${e.year})</div>
        <div class="d-flex gap-2">
          <button class="btn btn-${e.isBorrowed?`warning`:`primary`} btn-sm ${e.isBorrowed?`return`:`borrow`}-btn" data-id="${e.id}">${e.isBorrowed?`Повернути`:`Позичити`}</button>
          <button class="btn btn-outline-danger btn-sm delete-book-btn" data-id="${e.id}">Видалити</button>
        </div>
      </div>`).join(``),this.renderPagination(`book-pagination`,r,i,e=>{this.currentBookPage=e,this.renderBooks()})}handleBorrowBook(e){let t=this.bookLibrary.findById(e);t&&o.prompt(`Введіть ID користувача для позичення книги:`,`ID користувача`,e=>{let n=this.userLibrary.findById(e);if(!n)return o.show(`Помилка`,`Користувача з ID ${e} не знайдено!`);if(!n.canBorrow())return o.show(`Обмеження`,`Користувач ${n.name} вже позичив максимально можливу кількість книг (3)!`);t.borrow(n.id),n.addBook(t.id),this.saveData(),this.updateLists(),o.show(`Успішно`,`${t.title} by ${t.author} (${t.year}) has been borrowed by ${n.id} ${n.name} (${n.email}).`)})}handleReturnBook(e){let t=this.bookLibrary.findById(e);t&&(t.borrowedBy&&this.userLibrary.findById(t.borrowedBy)?.removeBook(t.id),t.returnBook(),this.saveData(),this.updateLists(),o.show(`Повернення`,`${t.title} by ${t.author} (${t.year}) has been returned.`,`Закрити`))}renderUsers(){let e=document.getElementById(`user-list-container`);if(!e)return;let{items:t,totalPages:n,page:r}=this.getPaginatedItems(this.userLibrary.getAll(),this.currentUsersPage);this.currentUsersPage=r,e.innerHTML=t.length===0?`<p class="text-muted">Користувачів немає.</p>`:t.map(e=>`
      <div class="d-flex justify-content-between align-items-center border-bottom py-2">
        <div><strong>${e.id}</strong> ${e.name} (${e.email}) [Позичено книг: ${e.borrowedBookIds.length}/3]</div>
        <button class="btn btn-outline-danger btn-sm delete-user-btn" data-id="${e.id}">Видалити</button>
      </div>`).join(``),this.renderPagination(`user-pagination`,n,r,e=>{this.currentUsersPage=e,this.renderUsers()})}renderPagination(e,t,n,r){let i=document.getElementById(e);if(!i)return;if(t<=1){i.innerHTML=``;return}let a=``;for(let e=1;e<=t;e++)a+=`<button class="btn ${e===n?`btn-primary`:`btn-outline-primary`} btn-sm page-btn" data-page="${e}">${e}</button>`;i.innerHTML=a,i.onclick=e=>{let t=e.target.closest(`.page-btn`);t&&r(parseInt(t.getAttribute(`data-page`)||`1`,10))}}};document.addEventListener(`DOMContentLoaded`,()=>{let e=document.getElementById(`app`);e&&new s().init(e)});